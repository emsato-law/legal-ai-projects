...
PDFs are archival. They're kept as provenance siblings, but the working source must be extracted text (`.md`, `.mmd`, `.txt`). Markdown is diffable, searchable, version-controllable, and parseable. A PDF is none of these things reliably — especially for Thai legal text, where PDF extraction routinely produces untrustworthy garbled output (See translation-pipeline for my solution to this).
